<template>
	<view class="page_disease_control diy_detail diy_detail--new" id="disease_control_details">
		<view class="warp">
			<view class="container-fluid">
				<view class="row">
					<view v-if="$check_field('get','title_name')" class="col-12 col-6">
						<view class="view">
					...
					<view v-if="$check_field('get','disease_type')" class="col-12 col-6">
						<view class="view">
							<view class="diy_title">
								<span>疾病类型</span>
							</view>
					...
					<view v-if="$check_field('get','symptom_response')" class="col-12 col-6">
						<view class="view">
							<view class="diy_title">
								<span>症状反应</span>
							</view>
								<view class="diy_field diy_text">
								<span>
									{{ obj["symptom_response"] }}
								</span>
							</view>
							</view>
					</view>
					<view v-if="$check_field('get','recommended_measure')" class="col-12 col-6">
						<view class="view">
							<view class="diy_title">
								<span>建议措施</span>
							</view>
...
<!--						<button type="button" class="btn btn-primary" @click="cancel()">返回</button>-->
						...
						<navigator class="btn link"
							:url="'/pages/comment/edit?source_table=disease_control&source_field=disease_control_id&source_id=' + obj['disease_control_id']">
							评论
						</navigator>
		...
		methods: {
			...
			get_praise(obj) {
				...
				this.$get("~/api/praise/count?", {
					source_table: "disease_control",
					source_field: "disease_control_id",
					source_id: obj["disease_control_id"]
				}, (res) => {
			...
			change_praise(obj) {
				...
				var query = {
					source_table: "disease_control",
					source_field: "disease_control_id",
					source_id: this.obj["disease_control_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/disease_control/set?disease_control_id=' + _this.obj["disease_control_id"], {
								praise_len
							}, (res) => {
								if(res.result){
