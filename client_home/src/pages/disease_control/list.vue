<template>
	<view class="page_disease_control diy_list diy_list_page div_list_tml" id="disease_control_list">
		<!-- 筛选模块(开始) -->
		<view>
			<view class="container">
				<view>
					<view>
						<view class="">
							<!-- 搜索栏 -->
													<uni-search-bar placeholder="搜索标题名称" @confirm="search" @cancel="cancel" cancelText="取消"
											@input="input($event, 'title_name')">
								<uni-icons slot="searchIcon" color="#999999" size="18" type="home" />
							</uni-search-bar>
																				<uni-search-bar placeholder="搜索症状反应" @confirm="search" @cancel="cancel" cancelText="取消"
											@input="input($event, 'symptom_response')">
								<uni-icons slot="searchIcon" color="#999999" size="18" type="home" />
							</uni-search-bar>
														<!-- /搜索栏 -->
						</view>
			...
			<view class="container">
				<view v-for="(o,i) in list" class="row">
					...
					<view class="uni-padding-wrap uni-common-mt">
						<navigator class="btn btn-primary" :url="'./details?disease_control_id=' + o['disease_control_id']">查看详情</navigator>
					</view>
				</view>
			</view>
		</view>
...
<script>
	 ...
	 */
	get_praise(list){
		this.$get("~/api/praise/count_group?groupby=source_id&source_table=disease_control", {}, (res) => {
			if(res.result){
				res.result.map((o)=>{
					for(var i = 0;i < list.length; i++){
						var oj = list[i];
						if(oj["disease_control_id"] === o["source_id"]){
