<template>
	<view class="page_traditional_chinese_medicine_classics diy_detail diy_detail--new" id="traditional_chinese_medicine_classics_details">
		<view class="warp">
			<view class="container-fluid">
				<view class="row">
					<view v-if="$check_field('get','name_of_classics')" class="col-12 col-6">
						<view class="view">
					...
					<view v-if="$check_field('get','types_of_classics')" class="col-12 col-6">
						<view class="view">
							<view class="diy_title">
								<span>典籍类型</span>
							</view>
					...
					<view v-if="$check_field('get','author_name')" class="col-12 col-6">
						<view class="view">
							<view class="diy_title">
								<span>作者名称</span>
							</view>
					...
					<view v-if="$check_field('get','picture_of_classics')" class="col-12 col-6">
						<view class="view">
							<view class="diy_title">
								<span>典籍图片</span>
							</view>
								<view class="diy_field diy_img">
								<img :src="$fullUrl(obj['picture_of_classics'])" />
							</view>
							</view>
					</view>
					<view v-if="$check_field('get','appendix_to_classic_books')" class="col-12 col-6">
						<view class="view">
							<view class="diy_title">
								<span>典籍附件</span>
							</view>
								<view class="diy_field diy_file">
								<button @click="download_file($fullUrl(obj['appendix_to_classic_books']))" class="btn" v-if="obj['appendix_to_classic_books']">下载文件</button>
							</view>
							</view>
					</view>
					<view v-if="$check_field('get','introduction_to_classical_books')" class="col-12 col-6">
						<view class="view">
							<view class="diy_title">
								<span>典籍简介</span>
							</view>
...
<!--						<button type="button" class="btn btn-primary" @click="cancel()">返回</button>-->
						...
						<navigator class="btn link"
							:url="'/pages/comment/edit?source_table=traditional_chinese_medicine_classics&source_field=traditional_chinese_medicine_classics_id&source_id=' + obj['traditional_chinese_medicine_classics_id']">
							评论
						</navigator>
	...
	export default {
			...
			return {
				...
				obj: {
						"name_of_classics": "",
							...
							"author_name": "",
							"picture_of_classics": "",
							"appendix_to_classic_books": "",
							"introduction_to_classical_books": "",
						"hits": 0,
					"praise_len": 0,
					"traditional_chinese_medicine_classics_id": 0,
		...
		methods: {
			...
			get_praise(obj) {
				...
				this.$get("~/api/praise/count?", {
					source_table: "traditional_chinese_medicine_classics",
					source_field: "traditional_chinese_medicine_classics_id",
					source_id: obj["traditional_chinese_medicine_classics_id"]
				}, (res) => {
			...
			change_praise(obj) {
				...
				var query = {
					source_table: "traditional_chinese_medicine_classics",
					source_field: "traditional_chinese_medicine_classics_id",
					source_id: this.obj["traditional_chinese_medicine_classics_id"],
					user_id
				};
        ...
        _this.obj.praise_len = parseInt(_this.obj.praise_len)
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/traditional_chinese_medicine_classics/set?traditional_chinese_medicine_classics_id=' + _this.obj["traditional_chinese_medicine_classics_id"], {
								praise_len
							}, (res) => {
								if(res.result){
			...
			change_collect() {
				...
				if (this.state_collect) {
					...
					this.state_collect = true;
								query.title = this.obj.name_of_classics
																								query.img = this.obj.picture_of_classics
			...
			add_hits(obj) {
				obj["hits"] = obj["hits"] + 1;
				var hits = obj["hits"];
				this.$post('~/api/traditional_chinese_medicine_classics/set?traditional_chinese_medicine_classics_id=' + obj["traditional_chinese_medicine_classics_id"], {
					hits
				}, (res) => {
					if(res.result){
