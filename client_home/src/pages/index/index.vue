<template>
  <view id="home" class="home--new">
    ...
    <Card
      class="list_diy"
      title="中医典籍推荐"
      url="/pages/traditional_chinese_medicine_classics/list"
      :list="list_article"
      v-if="$check_action('/traditional_chinese_medicine_classics/list', 'get')"
    >
	  <list_traditional_chinese_medicine_classics :list="list_traditional_chinese_medicine_classics"></list_traditional_chinese_medicine_classics>
    ...
    <Card
      class="list_diy"
      title="饮食养生推荐"
      url="/pages/dietary_health_preservation/list"
      :list="list_article"
      v-if="$check_action('/dietary_health_preservation/list', 'get')"
    >
	  <list_dietary_health_preservation :list="list_dietary_health_preservation"></list_dietary_health_preservation>
...
<script>
			...
			import list_traditional_chinese_medicine_classics from "@/components/diy/list_traditional_chinese_medicine_classics.vue";
			import list_dietary_health_preservation from "@/components/diy/list_dietary_health_preservation.vue";
...
export default {
  ...
  methods: {
    ...
    toToggle() {
      this.isAdmin = !this.isAdmin;
		...
		*  获取中医典籍
		*/
		get_traditional_chinese_medicine_classics() {
			let url = "~/api/traditional_chinese_medicine_classics/get_list?";
									
			this.$get(url, {
											}, (json) => {
					if (json.result && json.result.list) {
					console.log("中医典籍" ,json.result.list);
					this.list_traditional_chinese_medicine_classics = json.result.list;
											this.list_traditional_chinese_medicine_classics.map((o)=>{
							o["praise_len"];
						});
						this.get_praise(this.list_traditional_chinese_medicine_classics ,"traditional_chinese_medicine_classics" ,"33881");
									}
			});
		},
    			/**
		*  获取饮食养生
		*/
		get_dietary_health_preservation() {
			let url = "~/api/dietary_health_preservation/get_list?";
									
			this.$get(url, {
											}, (json) => {
					if (json.result && json.result.list) {
					console.log("饮食养生" ,json.result.list);
					this.list_dietary_health_preservation = json.result.list;
											this.list_dietary_health_preservation.map((o)=>{
							o["praise_len"];
						});
						this.get_praise(this.list_dietary_health_preservation ,"dietary_health_preservation" ,"33882");
									}
			});
		},
  ...
  onShow() {
    					this.get_traditional_chinese_medicine_classics();
					this.get_dietary_health_preservation();
		    this.get_menu();
    this.get_slides();
			this.get_article();
    // this.get_link();
