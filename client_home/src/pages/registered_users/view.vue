          ...
          <view class="">
            <uni-forms :modelValue="form"  v-if="is_view()">
              ...
              <uni-forms-item v-if="$check_field('get','user_age') || $check_field('add','user_age') || $check_field('set','user_age')" label="用户年龄" name="user_age">
                            <uni-easyinput type="text" v-model="form['user_age']" v-if="user_group === '管理员' || (form['registered_users_id'] && $check_field('set','user_age')) || (!form['registered_users_id'] && $check_field('add','user_age'))" :disabled="disabledObj['user_age_isDisabled']" />
                <!-- 仅查看 -->
