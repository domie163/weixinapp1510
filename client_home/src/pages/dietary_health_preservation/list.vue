<template>
	<view class="page_dietary_health_preservation diy_list diy_list_page div_list_tml" id="dietary_health_preservation_list">
		<!-- 筛选模块(开始) -->
		<view>
			<view class="container">
				<view>
					<view>
						<view class="">
							<!-- 搜索栏 -->
													<uni-search-bar placeholder="搜索养生标题" @confirm="search" @cancel="cancel" cancelText="取消"
											@input="input($event, 'health_title')">
								<uni-icons slot="searchIcon" color="#999999" size="18" type="home" />
							</uni-search-bar>
																			<uni-search-bar placeholder="搜索养生类型" @confirm="search" @cancel="cancel" cancelText="取消"
											@input="input($event, 'type_of_health_preservation')">
								<uni-icons slot="searchIcon" color="#999999" size="18" type="home" />
							</uni-search-bar>
																	<!-- /搜索栏 -->
						</view>
			...
			<view class="container">
				<view v-for="(o,i) in list" class="row box_style">
					...
					<view class="uni-padding-wrap uni-common-mt">
						<navigator :url="'./details?dietary_health_preservation_id=' + o['dietary_health_preservation_id']" class="btn btn-primary">查看详情</navigator>
...
<script>
	 ...
	 */
	get_praise(list){
		this.$get("~/api/praise/count_group?groupby=source_id&source_table=dietary_health_preservation", {}, (res) => {
			if(res.result){
				res.result.map((o)=>{
					for(var i = 0;i < list.length; i++){
						var oj = list[i];
						if(oj["dietary_health_preservation_id"] === o["source_id"]){
