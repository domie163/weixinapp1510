<template>
	<view class="page_dietary_health_preservation diy_detail diy_detail--new" id="dietary_health_preservation_details">
		<view class="warp">
			<view class="container-fluid">
				<view class="row">
					<view v-if="$check_field('get','health_title')" class="col-12 col-6">
						<view class="view">
					...
					<view v-if="$check_field('get','type_of_health_preservation')" class="col-12 col-6">
						<view class="view">
							<view class="diy_title">
								<span>养生类型</span>
							</view>
					...
					<view v-if="$check_field('get','health_preservation_methods')" class="col-12 col-6">
						<view class="view">
							<view class="diy_title">
								<span>养生方法</span>
							</view>
					...
					<view v-if="$check_field('get','health_preservation_plan')" class="col-12 col-6">
						<view class="view">
							<view class="diy_title">
								<span>养生方案</span>
							</view>
...
<!--						<button type="button" class="btn btn-primary" @click="cancel()">返回</button>-->
						...
						<navigator class="btn link"
							:url="'/pages/comment/edit?source_table=dietary_health_preservation&source_field=dietary_health_preservation_id&source_id=' + obj['dietary_health_preservation_id']">
							评论
						</navigator>
	...
	export default {
			...
			return {
				...
				obj: {
						"health_title": "",
							...
							"cover_photo": "",
							"release_date": "",
							"health_preservation_methods": "",
							"health_preservation_plan": "",
						"hits": 0,
					"praise_len": 0,
					"dietary_health_preservation_id": 0,
		...
		methods: {
			...
			get_praise(obj) {
				...
				this.$get("~/api/praise/count?", {
					source_table: "dietary_health_preservation",
					source_field: "dietary_health_preservation_id",
					source_id: obj["dietary_health_preservation_id"]
				}, (res) => {
			...
			change_praise(obj) {
				...
				var query = {
					source_table: "dietary_health_preservation",
					source_field: "dietary_health_preservation_id",
					source_id: this.obj["dietary_health_preservation_id"],
					user_id
				};
        ...
        _this.obj.praise_len = parseInt(_this.obj.praise_len)
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/dietary_health_preservation/set?dietary_health_preservation_id=' + _this.obj["dietary_health_preservation_id"], {
								praise_len
							}, (res) => {
								if(res.result){
			...
			change_collect() {
				...
				if (this.state_collect) {
					...
					this.state_collect = true;
								query.title = this.obj.health_title
			...
			add_hits(obj) {
				obj["hits"] = obj["hits"] + 1;
				var hits = obj["hits"];
				this.$post('~/api/dietary_health_preservation/set?dietary_health_preservation_id=' + obj["dietary_health_preservation_id"], {
					hits
				}, (res) => {
					if(res.result){
